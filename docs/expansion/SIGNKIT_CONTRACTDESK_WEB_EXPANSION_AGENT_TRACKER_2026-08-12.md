...
| B-18 | implicit | Desktop/PDF Runtime | DONE | Integrated the reusable isolated PDFium worker into the canonical local-companion `/workspace/executions/{id}/document-inspections` route. The route bounds input, rejects cloud topology, validates with pikepdf, executes field inspection in a disposable PDFium process, stores only hash/result metadata, deletes temporary bytes, and reconciles idempotent replay/conflict. Browser binding is in `web/cloud_workspace`; route integration evidence is S2/S3-sensitive Tier 3. | B-17, A-1 |
...
Confidence for this stage: **0.97** (fresh-context metadata proof plus a Tier 3 local-companion route-to-worker-to-receipt path now exist; hosted integration, production trust, and browser manual upload evidence remain separate gates).
