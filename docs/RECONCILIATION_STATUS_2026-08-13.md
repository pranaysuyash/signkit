...
| RECON-30 | open | calibration artifact preservation and release reproducibility | A concurrent calibration slice now generates manifests, reports, and dataset assets, but the current `/datasets/` ignore rule also hides the manifests and reports that its documentation describes as reproducible inputs. The slice is preserved uncommitted and was not staged by this reconciliation. | Decide and implement an explicit artifact policy: commit small manifests and reports, retain generated assets through a documented reproducible build or approved fixture archive, ensure `.gitignore` cannot hide required provenance, and rerun the calibration proof from a clean checkout. |
