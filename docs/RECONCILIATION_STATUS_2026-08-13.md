...
| RECON-06 | done-local | workspace tooling and agent-start | The documented local workspace-memory setup produced Python `3.13.3` and `memsearch 0.4.17`; a real SignKit sync/index/search processed `588` files and `16042` chunks, full retrieval completed with `16047` project-collection chunks, and bounded forced retrieval returned `0`. Generated context retained the project-local motto SHA and truthfully recorded unavailable shared-collection status. Evidence: QA-45 and `docs/review/agent_start_retrieval_runtime_proof_2026-08-14.md`. | Keep the source-selection/retention guard and rerun retrieval after wrapper or corpus changes. Full all-project retrieval quality, shared-collection population, provider portability, hosted execution, and regeneration after the concurrent calibration slice is reconciled remain open. |
...
## Addendum (2026-08-14): calibration artifact policy

RECON-30 is closed locally by the tracked manifest/report/note boundary,
deterministic builder metadata, generation-metadata validation, and QA-49.
The first checklist entry recorded `9` focused checks; the later loader
validation expanded the current evidence to `10` checks. The artifact proof at
`docs/review/calibration_artifact_policy_proof_2026-08-14.md` is authoritative
...
## Addendum (2026-08-14): provider decision research refresh

`L2-15` records a primary-source refresh of the Gumroad and Dodo Payments
license and entitlement models. The memo distinguishes provider capabilities
from SignKit configuration and makes the remaining selection, activation,
