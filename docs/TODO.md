...
- [~] `RECON-30` Resolve calibration artifact preservation: the concurrent `/datasets/` ignore rule currently hides manifests and reports that the calibration documentation describes as reproducible inputs. Define the commit/archive/regeneration boundary, make provenance visible, and rerun the calibration proof from a clean checkout.
