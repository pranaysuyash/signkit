...
| Route | Legacy and direct HTML paths redirect to `/` | 301 responses with query preservation | Web Platform | Block release |
...
| Checkout | Provider state is configuration-driven | Config inspection and browser smoke | Platform | Block release |
...
python3 -m py_compile serve.py tools/audit_public_surface.py
