...
| QA-53 | PASS with boundary | Tier 3 disposable local migration and authenticated extraction smoke, S2/S3 | `./.venv/bin/python tools/run_extraction_hosted_smoke.py` passed against a temporary SQLite database after applying Alembic head `9c4b7e2d1a6f`. The run covered health `200`, registration/login, authenticated upload, durable replay, cross-owner selection denial, selection, processing, export manifest, deletion cleanup, post-delete audit events, and cross-owner deletion denial. `tests/test_extraction_hosted_smoke_tool.py` passed, the health-gate mutation was killed, and the complete mutation manifest passed `17/17`. This is local contract evidence, not target-database migration, hosted, rollback, provider, or production operator evidence. See `docs/review/local_extraction_ownership_smoke_proof_2026-08-14.md`. |
| QA-54 | PASS with boundary | Tier 2 disposable Alembic upgrade, downgrade, and re-upgrade, S2/S3 | `./.venv/bin/python tools/run_local_migration_recovery_proof.py` passed with initial and final head `9c4b7e2d1a6f` and rollback target `e42b7f8c91aa`; receipt fields and the request-hash index were removed and restored as expected. The subprocess test passed, the rollback-target mutation was killed, and the complete mutation manifest passed `18/18`. This is local SQLite migration evidence, not target-database backup restoration, hosted rollback, or live operator recovery. See `docs/review/local_migration_recovery_proof_2026-08-14.md`. |
...
| QA-60 | PASS with boundary | Tier 2 full-roadmap synchronization contract, S1 | `tests/test_todo_truth_contract.py` also verifies that `docs/TODO_FULL.md` is a preserved roadmap inventory rather than a competing authority, reflects QA-proven local closures, and retains genuinely pending external and product work. See `docs/review/todo_truth_contract_proof_2026-08-14.md`. |
| QA-61 | PASS with boundary | Tier 4 local Browser Daemon operator observation, S1 | A fresh isolated local stack observation used `tools/run_local_product_browser_proof.mjs` plus the persistent Browser Daemon. The canonical landing passed title, `h1`, five-state Source/Mark/Clean/Place/Ready rail, local workspace handoff, landmarks, responsive no-overflow, reduced motion, and zero browser errors at 1440x900, 390x844, and 320x844. The workspace passed HTTP 200, auth/app shells, main landmark, and explicit metadata-only non-signing boundary copy. The stack shut down with ports 8080 and 8001 closed. Packaged/cross-platform stale-state, assistive-technology, hosted, and real-user evidence remain separate. See `docs/review/local_operator_browser_observation_2026-08-14.md`. |
| QA-62 | PASS with boundary | Tier 4 local optional-analytics contract and Browser Daemon observation, S1 | `./.venv/bin/python -m pytest tests/test_landing_analytics_contract.py -q` passed `3` tests, and the complete canonical suite passed `538 passed, 4 skipped`. The missing-`gtag` case runs bot detection and asserts no calls or console records; the configured-`gtag` case forwards `real_user_detected` and the expected `set` call without a synthetic `REMOTE_EVENT` log; the asset-version case guards cache invalidation on the canonical root. The cache-versioned landing loaded in the persistent Browser Daemon with zero console entries, and `node tools/run_local_product_browser_proof.mjs` passed at desktop, touch, and narrow viewports with zero browser errors. This is local JavaScript boundary evidence only and does not prove provider activation, consent, hosted parity, event delivery, or production observability. See `docs/review/local_analytics_boundary_proof_2026-08-14.md`. |
