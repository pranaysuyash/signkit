...
| QA-41 | PASS with boundary | Canonical local full suite, S1 | `./.venv/bin/python -m pytest -q` passed `520 passed, 4 skipped` after the agent-start contract and calibration slice. The four skips remain explicit for optional PyMuPDF and Qt event-loop boundaries. This is local evidence only and does not substitute for remote CI, packaged/cross-platform, hosted, provider, device, or assistive-technology proof. |
