...
Configuration status legend: `REQUIRED` values must be replaced before a local
backend starts; `LOCAL DEFAULT` values are safe development defaults;
`OPTIONAL` integrations are disabled when blank or false; `HOSTED ONLY` values
require deployment, migration, and operator evidence before being used in a
