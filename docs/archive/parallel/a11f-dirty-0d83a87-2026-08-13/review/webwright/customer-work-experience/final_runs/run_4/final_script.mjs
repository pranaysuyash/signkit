...
const log = (step, message) => appendFileSync(logPath, `step ${step} action: ${message}\n`);
...
try {
  ...
  if (present.length || await page.title() !== "SignKit | Get the document right") throw new Error(`Public language/title check failed: ${present.join(", ")}`);
  log(1, "opened customer-facing concept; title and public-language guard passed; compact hero screenshot saved");
  ...
  if (selected !== "true" || alt !== "Current SignKit PDF workspace with a completed sample workflow" || !caption.toLowerCase().includes("completed sample workflow")) throw new Error("Workflow stage did not update correctly");
  await page.screenshot({ path: join(screenshots, "final_execution_2_pdf_workflow_selected.png") });
  log(2, "selected completed-PDF workflow state; selected tab, accessible image label, caption, and compact stage layout verified");

  if (errors.length) throw new Error(errors.join("; "));
  log(3, "verified no browser console or page errors after interaction");
  appendFileSync(logPath, "\nFINAL_RESPONSE: Customer-facing concept opened, visual evidence captured, PDF workflow interaction verified, compact layout inspected, and no console errors observed.\n");
