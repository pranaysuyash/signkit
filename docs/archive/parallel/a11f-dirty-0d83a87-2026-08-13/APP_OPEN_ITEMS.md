...
## Addendum (2026-08-13): release evidence ledger

The versioned-artifact/checksum portion of the packaging item is now in
progress through `tools/release_artifact_ledger.py` and the tagged release job.
The ledger records checksums and release metadata, but the item remains open
until signing or notarization, platform smoke, rollback evidence, and a real
tagged workflow run are recorded. Changelog ownership is still pending.
