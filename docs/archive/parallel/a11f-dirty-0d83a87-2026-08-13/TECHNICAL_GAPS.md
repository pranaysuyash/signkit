...
### 5. QA Runbook
- No recorded clean macOS/Windows/Linux VM smoke results. *(The reproducible
  local QA matrix now covers EXIF photos, large scans, tiny selections, invalid
  formats, and offline backend; clean-install/device evidence remains open.)*
...
- The active backend configuration now reads the repository-root `.env`, has an
  explicit `DATABASE_URL` field, and uses SQLite as the documented local
  default. `scripts/run-backend-dev.sh` no longer rejects that documented local
  setup before settings are loaded; `BACKEND_HOST` and `BACKEND_PORT` remain
  configurable with 127.0.0.1:8001 defaults.
- `docs/QA_CHECKLIST.md` and `docs/QA_RESULTS.md` now provide the reproducible
  local matrix requested by the launch backlog. The local negative paths,
  offline behavior, EXIF orientations, and configuration contract passed in the
  canonical `.venv`; target deployment, clean-install/device, migration,
  provider, and hosted proof remain open.
- The desktop EXIF loader now uses Pillow's canonical all-orientation
  transpose and passes an explicit RGB scanline stride to QImage. This closes
  the mirrored-orientation gap and a width-not-divisible-by-four pixel-layout
  defect found by the new QA test.
- The extraction upload route now returns 415 for an explicitly unsupported
  media type before writing an artifact. Extension, magic-number, size,
  dimension, and malformed-image validation remain separate 400-class checks.
- The canonical `test-data` CI workflow now executes the release QA matrix,
  authenticated extraction smoke, public-surface checks, local deployment
  smoke, and curated mutation gate. Local CI-equivalent evidence is green, but
  a GitHub-hosted run receipt is still required before describing CI as closed.
