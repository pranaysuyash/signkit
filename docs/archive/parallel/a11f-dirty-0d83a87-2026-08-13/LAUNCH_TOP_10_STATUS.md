...
## 10. QA Matrix [x] LOCAL MATRIX COMPLETE, HOSTED GATES OPEN
...
- [x] Large payload and dimension rejection tests
- [x] EXIF orientations 1 through 8, including mirrored variants
- [x] Tiny selections below 10 px
- [x] Bad inputs with 415/404/422 contracts
- [x] Offline mode without an HTTP request
- [x] Reproducible pass list and known-limit table
- [ ] Clean-install/device matrix and narrow viewport pass
- [ ] Hosted deployment, migration/recovery, and provider purchase evidence

**Evidence:** `docs/QA_CHECKLIST.md` and `docs/QA_RESULTS.md`. The local
matrix is evidence-backed, but the hosted and provider rows remain release
blockers. The external `signkit.work` probe is still failing its canonical
marker, redirect, and JavaScript content-type checks.
