...
| L1-08 | implicit | operator workflow | Establish one operator-facing state and recovery contract across desktop extraction, PDF placement, Vault, export, and workspace surfaces | in-progress | P1 | The fresh Tier 4 local proof observes extraction, Vault, forced failure, retry, passport, and verified visual placement; malformed-input, timeout, partial-export, deletion-cleanup, and local-companion-outage states still need scoped controls and evidence | `docs/PRODUCT_GLOSSARY.md`, `docs/STATE_CONTENT_MATRIX.md`, `docs/review/local_operator_state_proof_2026-08-13.md`, `desktop_app/views/`, `desktop_app/workflows/`, `QA-28` | Product + eng |
