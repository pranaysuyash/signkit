...
| RECON-30 | implicit | calibration artifact preservation and release reproducibility | open | P1 | The concurrent calibration slice generates manifests, reports, and dataset assets, but the current `/datasets/` ignore rule also hides manifests and reports described as reproducible inputs. The slice remains preserved and unstaged. | Decide and implement an explicit artifact policy, keep required provenance visible to Git, retain or reproducibly regenerate generated assets, and rerun calibration from a clean checkout. |
