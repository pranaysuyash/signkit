...
| RECON-22 | implicit | local product / auto-detection safety | done-local | P1 | The existing ranked `auto_detect_signatures` API is now surfaced through `SignatureCandidateDialog`; the operator can inspect a clipped candidate preview and must confirm a candidate before selection is applied. Focused candidate-picker and detection tests pass. | Preserve explicit human confirmation and the non-probabilistic score label. Reopen for recall@k/IoU evaluation only after a labeled, permissioned corpus and an agreed accuracy bar exist; ML/cloud detection remains deferred. |
