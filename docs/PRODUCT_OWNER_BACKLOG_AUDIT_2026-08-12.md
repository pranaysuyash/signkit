...
| L0-05 | explicit | release | Publish evidence-backed artifact ledger (release SHA, package filename, checksum, signing/notarization, smoke output) | in-progress | P0 | Current `main` now has a rebuilt macOS arm64 app and DMG identity, frozen runtime proof, real local browser and bridge proof, and a schema-valid ledger whose strict readiness gate still blocks on verified signing and rollback; other-platform, clean-machine, hosted, provider, and remote-CI evidence remain required | `tools/release_artifact_ledger.py`, `docs/review/local_packaged_artifact_proof_2026-08-14.md`, `QA-65`, `.codex-test-tmp/current-arm64-release-ledger.json` | PO + eng |
...
| L2-16 | implicit | research/quality | Keep synthetic calibration evidence reproducible in clean CI and fail on detector or generator baseline drift | done-local | `tools/run_calibration_regression.py` regenerates ignored PNG/PDF fixtures from tracked manifest metadata, runs the real image and PDF adapters with isotonic and Platt calibration, and requires all four generated reports to match tracked baselines; local execution passed and the CI workflow now runs the gate | `tools/run_calibration_regression.py`, `.github/workflows/test-data.yml`, `tests/test_calibration_regression_contract.py`, `docs/calibration_dataset_spec.md`, `QA-66`, `RECON-32` | eng + QA |
...
| L0-14 | implicit | release | Create a per-artifact release ledger with checksum, signing, smoke, and rollback evidence | in-progress | P0 | Current local arm64 ledger identity and smoke evidence are refreshed at QA-65; the strict release gate correctly blocks `not_verified` signing and `not-recorded` rollback, and the per-platform, notarization, hosted, and remote-CI gates remain open | `tools/release_artifact_ledger.py`, `tests/test_release_artifact_ledger.py`, `docs/release/RELEASE_ARTIFACT_LEDGER_SPEC.md`, `docs/review/local_packaged_artifact_proof_2026-08-14.md`, `QA-65`, `.github/workflows/build-all-platforms.yml` | eng + release owner |
...
| RECON-32 | implicit | research/quality | Add a clean-checkout regression gate for tracked synthetic calibration manifests and detector outputs | done-local | The reusable regression command recreates ignored assets, validates builder metadata and manifest identity, runs both real detector adapters with isotonic and Platt calibration, and rejects any report drift; focused contract and local four-report execution pass. Remote CI execution remains a separate delivery gate. | `tools/run_calibration_regression.py`, `.github/workflows/test-data.yml`, `tests/test_calibration_regression_contract.py`, `docs/review/calibration_ci_regression_proof_2026-08-14.md`, `L2-16` | eng + QA |
