...
| RECON-23 | implicit | local product / desktop observation | open | P1 | Offscreen Qt contract evidence exists for the candidate picker, but no current real-GUI observation is attached to the changed flow. | Run the reusable desktop GUI proof against a disposable image, observe candidate selection, cancel, confirm, keyboard focus, and failure messaging, then attach the output and keep assistive-technology limitations explicit. |
| RECON-24 | implicit | research / auto-detection evaluation | open | P1 | `tools/evaluate_signature_corpus.py` and synthetic fixtures exist, but no permissioned labeled corpus or measured production-like recall@k/IoU baseline exists. | Register a permissioned labeled corpus with provenance/split rules, run a held-out baseline, record recall@k/IoU and failure classes, and decide whether the threshold/default can be promoted. Do not collect user documents without explicit consent and retention/deletion rules. |
